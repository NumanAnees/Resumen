...
class Welcome extends Component {
  constructor(props) {
    ...
    if (this.currentResume !== null) {
      ...
      this.currentResume.languages = this.checkForNullsInArray(
        this.currentResume.languages,
        null
      );
    }
    this.state = {
      ...
      resumeName:
        ...
        this.currentResume.item.template !== undefined
          ? this.currentResume.item.template
          : "Cv1",
      currentResumeName: "Cv1",
      currentResume: null,
      title:
        this.currentResume != null &&
        this.currentResume.item.title !== undefined
          ? this.currentResume.item.title
          : "Untitled",
      progress: 0,
      pages: [],
      firstname:
      ...
      languages:
        this.currentResume !== null &&
        this.currentResume.languages !== undefined
          ? this.currentResume.languages
          : [],
    ...
    fire.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        getUserMembership(this.state.user.uid).then((value) => {
          ...
          if (
            value.membershipEnds != undefined &&
            typeof value.membershipEnds != "string"
          ) {
            this.setState({
  ...
  componentDidMount() {
    ...
    getSubscriptionStatus().then((data) =>
      this.setState({ subscriptionsStatus: data.state })
    );
  }
  ...
  handleInputs(inputName, inputValue, idOptional, typeOptional) {
    ...
    switch (inputName) {
      ...
      case "First Name":
        ...
        inputValue == "" &&
          this.setState({ fullFields: this.state.fullFields - 1 });
        break;
      case "Last Name":
        this.state.lastname == ""
          ? this.setState({
      ...
      case "Photo":
        this.state.photo == ""
          ...
          : this.setState({ photo: inputValue });
        inputValue == "" &&
          this.setState({ fullFields: this.state.fullFields - 1 });
        break;
      case "Occupation":
        ...
        {this.state.loaded && (
          <>
            <Helmet>
              ...
              <meta
                name="description"
                content={this.state.websiteDescription}
              />
                ...
                >
                  <ul>
                    <li
                      ...
                      onClick={() => this.handleMenuLink("Home")}
                    >
                      Home <a></a>
                    </li>
